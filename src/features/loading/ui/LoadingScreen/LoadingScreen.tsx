...
export function LoadingScreen({
  ...
  return visible ? (
    <motion.div
      ...
      initial={false}
      animate={{ opacity: phase === 'fading' || phase === 'skipping' ? 0 : 1 }}
      transition={{
        duration:
          phase === 'fading'
            ? loadingAnimationConfig.loaderFadeDuration /
              Math.max(animationSpeed, 0.01)
            : phase === 'skipping'
              ? loadingAnimationConfig.skipFadeDuration /
                Math.max(animationSpeed, 0.01)
              : 0,
      }}
      onAnimationComplete={() => {
        if (phase === 'fading') onFadeComplete()
        if (phase === 'skipping') onSkipComplete()
      }}
    ...
    >
      ...
      <div className={cx(styles.loadingTerminal)}>
        ...
        <header className={cx(styles.loadingTerminalHeader)}>
          ...
          <span className={cx(styles.loadingTerminalCode)}>NODE: EU-07</span>
        </header>
        <div className={cx(styles.loadingStatusRow)}>
          <span className={cx(styles.loadingStatusDot)} aria-hidden="true" />
          <span>{statusText}</span>
          <span className={cx(styles.loadingTerminalCode)}>QUERY: #8F72C1</span>
        </div>
        <div className={cx(styles.loadingQueryWrap)}>
          ...
          <div className={cx(styles.loadingQueryLine)}>
            ...
            />
            <span className={cx(styles.loadingQueryDisplay)} aria-hidden="true">
              <span>{queryText}</span>
              {phase !== 'initializing' && (
                <span
                  className={cx(styles.typingCursor, styles.loadingQueryCursor)}
                >
                  |
                </span>
              )}
        ...
        {phase === 'searching' && (
          <div className={cx(styles.loadingCandidatesScan)} aria-live="polite">
            <span
        ...
        {allReady && phase !== 'fading' && phase !== 'skipping' && (
          <button
